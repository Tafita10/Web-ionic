...
export const useAuthStore = defineStore('auth', () => {
  ...
  async function login(credentials: LoginCredentials | { email: string; password: string }) {
    ...
    try {
      ...
      if (isOnline.value && FirebaseService.isInitialized()) {
        ...
        const response = await ApiService.post<{ token: string; user: User }>('/auth/login', { email, password }) as any;
  ...
  async function updateProfile(data: { nom: string; prenom: string; telephone: string; adresse: string }) {
    loading.value = true;
    try {
      const response = await ApiService.put<User>('/auth/profile', data) as any;
      if (response) {
